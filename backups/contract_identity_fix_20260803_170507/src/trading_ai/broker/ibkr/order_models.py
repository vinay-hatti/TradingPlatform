...
class IbkrPaperOrderRequest:
    ...
    def validate(self) -> None:
        ...
        if self.security_type.upper() == "OPT":
            if self.contract_id <= 0:
                raise ValueError("qualified option contract_id must be positive")
            if not self.local_symbol.strip():
                raise ValueError("qualified option local_symbol is required")
            if not self.expiry.strip():
                raise ValueError("option expiry is required")
            if self.strike is None or float(self.strike) <= 0:
