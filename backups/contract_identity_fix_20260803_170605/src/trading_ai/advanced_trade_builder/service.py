...
class AdvancedTradeBuilderService:
 ...
 def economics(legs:tuple[TradeLeg,...],capital:float,risk_pct:float):
  ...
  checks={'has_legs':bool(legs),'max_four_legs':len(legs)<=4,'positive_quantities':all(l.quantity>0 for l in legs),'positive_strikes':all(float(l.strike)>0 for l in legs),'positive_limit_prices':all(float(l.limit_price)>0 for l in legs),'single_expiry':len({l.expiry for l in legs})==1,'option_contract_identity_present':all(bool(str(l.option_symbol or '').strip()) for l in legs),'risk_within_budget':max_loss<=budget+1e-9,'defined_risk':len(legs)<=2 or net>0}
