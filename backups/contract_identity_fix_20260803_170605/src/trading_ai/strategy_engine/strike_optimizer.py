...
class StrikeOptimizer:
    ...
    def spread_candidates(
        ...
        filtered = [
            ...
            and self._valid_contract_row(r, underlying_price)
        ]
            ...
            for long_leg in filtered:
                ...
                candidates.append(
                    SpreadCandidate(
                        ...
                        allowed=allowed,
                        warnings=warnings,
                        short_option_symbol=str(self._get(short_leg, "option_symbol", "") or ""),
                        long_option_symbol=str(self._get(long_leg, "option_symbol", "") or ""),
                        short_contract_id=int(self._float(short_leg, "contract_id", self._float(short_leg, "conid", 0))),
                        long_contract_id=int(self._float(long_leg, "contract_id", self._float(long_leg, "conid", 0))),
                        short_bid=self._float(short_leg, "bid"),
                        short_ask=self._float(short_leg, "ask"),
                        short_last=self._float(short_leg, "last"),
                        short_mid=short_mid,
                        short_volume=int(self._float(short_leg, "volume")),
                        short_open_interest=int(self._float(short_leg, "open_interest")),
                        short_gamma=self._float(short_leg, "gamma"),
                        short_theta=self._float(short_leg, "theta"),
                        short_vega=self._float(short_leg, "vega"),
                        short_rho=self._float(short_leg, "rho"),
                        short_implied_volatility=self._float(short_leg, "implied_volatility"),
                        long_bid=self._float(long_leg, "bid"),
                        long_ask=self._float(long_leg, "ask"),
                        long_last=self._float(long_leg, "last"),
                        long_mid=long_mid,
                        long_volume=int(self._float(long_leg, "volume")),
                        long_open_interest=int(self._float(long_leg, "open_interest")),
                        long_gamma=self._float(long_leg, "gamma"),
                        long_theta=self._float(long_leg, "theta"),
                        long_vega=self._float(long_leg, "vega"),
                        long_rho=self._float(long_leg, "rho"),
                        long_implied_volatility=self._float(long_leg, "implied_volatility"),
                    )
                )
    ...
    def _valid_contract_row(self, row, underlying_price: float) -> bool:
        option_symbol = str(self._get(row, "option_symbol", "") or "").strip()
        expiry = str(self._get(row, "expiry", "") or "").strip()
        strike = self._float(row, "strike")
        if not option_symbol or not expiry or strike <= 0 or underlying_price <= 0:
            return False
        distance = abs(strike - underlying_price) / underlying_price
        return distance <= self.max_strike_distance_pct
