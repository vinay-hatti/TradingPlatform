...
class OpportunityFactory:
    ...
    def create(
        ...
        contract_legs = list(getattr(strike_candidate, "legs", []) or []) if strike_candidate is not None else []
        combined_metadata = {
            **dict(metadata or {}),
            "payoff_profile": payoff_profile,
            "probability_profile": probability_profile,
            "option_contract_legs": contract_legs,
            "contract_identity_complete": bool(
                contract_legs
                and all(str(leg.get("option_symbol") or "").strip() for leg in contract_legs)
            ),
        }
