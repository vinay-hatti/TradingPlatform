...
function TrendIntelligenceCandidateCard({trade}:{trade:any}) {
  ...
  const statuses=[trade.trend_context_status,trade.transition_context_status,trade.forecast_context_status,trade.institutional_context_status].filter(Boolean);
  const status=statuses.includes('ERROR')?'ERROR':statuses.includes('STALE')?'STALE':statuses.includes('MISSING')?'DEGRADED':statuses.every((value:string)=>value==='FRESH')?'FRESH':(statuses[0]||'NOT_AVAILABLE');
  ...
  const explanation=available
    ? `The ${direction} candidate is aligned ${numeric(trade.signal_trend_alignment_score,50).toFixed(1)}% with the persisted trend model. Base trend contributed ${numeric(trade.base_trend_score_adjustment).toFixed(2)} points, transition intelligence contributed ${numeric(trade.transition_score_adjustment).toFixed(2)}, forecast intelligence contributed ${numeric(trade.forecast_score_adjustment).toFixed(2)}, institutional intelligence contributed ${numeric(trade.institutional_score_adjustment).toFixed(2)}, and the governed total trend adjustment was ${total.toFixed(2)}.`
  ...
  return <div className="trend-candidate-card">
    ...
    <div className="trend-score-strip">
      ...
      <div><span>Total adjustment</span><strong className={total>=0?'positive-value':'negative-value'}>{total>=0?'+':''}{total.toFixed(2)}</strong><small>Base {numeric(trade.base_trend_score_adjustment).toFixed(2)} · Transition {numeric(trade.transition_score_adjustment).toFixed(2)} · Forecast {numeric(trade.forecast_score_adjustment).toFixed(2)} · Institutional {numeric(trade.institutional_score_adjustment).toFixed(2)}</small></div>
    </div>
