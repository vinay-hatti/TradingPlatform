...
function ScannerWorkspacePage({config}:{config:ScannerWorkspaceConfig}){
  ...
  const stageOpportunity=(trade:any)=>{const payload:OpportunityHandoff={handoffId:`option-opportunity-${Date.now()}`,stagedAt:new Date().toISOString(),source:'OPTION_SCANNER',scannerRunId:selected?.run_id||null,snapshotTimestamp:snapshot.timestamp,strategy,riskProfile,trade};sessionStorage.setItem('trading-ai:opportunity-handoff:current',JSON.stringify(payload));const historyRaw=localStorage.getItem('trading-ai:opportunity-handoff:history');let history:any[]=[];try{history=historyRaw?JSON.parse(historyRaw):[]}catch{history=[]}localStorage.setItem('trading-ai:opportunity-handoff:history',JSON.stringify([payload,...(Array.isArray(history)?history:[])].slice(0,50)));setHandoffMessage(`${trade.symbol||'Opportunity'} staged with scanner and snapshot provenance.`)};
