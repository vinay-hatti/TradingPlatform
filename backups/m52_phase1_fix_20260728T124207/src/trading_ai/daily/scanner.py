...
class DailyScanner:
    def __init__(
        ...
        maximum_trend_score_adjustment=12.0,
    ):
    ...
    def _trend_intelligence_context(self, *, symbol, signal):
        ...
        try:
            context = self.trend_intelligence_repository.scanner_context(
                symbol, signal, maximum_age_days=self.maximum_trend_snapshot_age_days
            )
    ...
    def scan_symbol(self, symbol):
        ...
        trend_adjustment = float(trend_context["trend_score_adjustment"])
        final_ai_score = max(0.0, min(100.0, base_ai_score + dealer_adjustment + trend_adjustment))
        (
        ...
        if trend_context["trend_context_status"] == "FRESH":
            ranking_reason += (
                ...
                f"adjustment={trend_adjustment:+.2f}, RS={trend_context['relative_strength_grade']}."
            )
