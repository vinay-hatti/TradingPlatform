...
SECTOR_ETFS={"Technology":"XLK","Financial Services":"XLF","Healthcare":"XLV","Consumer Cyclical":"XLY","Consumer Defensive":"XLP","Industrials":"XLI","Energy":"XLE","Basic Materials":"XLB","Real Estate":"XLRE","Utilities":"XLU","Communication Services":"XLC"}
...
class TrendIntelligenceService:
    ...
    def build(self, symbols=None, persist=True):
        ...
        data=self._prices(set(targets)|refs); results=[]; errors=[]
        ...
        return {'status':'READY' if results and not errors else 'DEGRADED' if results else 'FAILED','snapshot_timestamp':datetime.now(timezone.utc).isoformat(),'symbol_count':len(results),'error_count':len(errors),'results':[x.to_dict() for x in results],'errors':errors}
