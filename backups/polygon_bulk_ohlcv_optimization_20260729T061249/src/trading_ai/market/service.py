...
class MarketService:
    ...
    def _cache_is_fresh(
        ...
        if frame.empty:
            return False
        dates = cls._frame_dates(frame).dropna()
        if dates.empty:
            return False

        latest_bar = dates.max().date()
        expected = date.fromisoformat(requested_end)
        if expected >= date.today():
            expected -= timedelta(days=1)
        while expected.weekday() >= 5:
            expected -= timedelta(days=1)

        # Permit one missing weekday for an exchange holiday, but reject
        # multi-session stale cache coverage such as July 24 for July 29.
        prior_weekday = expected - timedelta(days=1)
        while prior_weekday.weekday() >= 5:
            prior_weekday -= timedelta(days=1)
        return latest_bar >= prior_weekday
    ...
    def _load_cached_df(
        ...
        for file in candidates:
            ...
            if sliced.empty or not self._cache_is_fresh(sliced, end):
                continue
        ...
        *,
        ...
        if cache_only:
            raise MarketDataNotCachedError(
                f"No fresh, non-empty cached Polygon market data covers {symbol} "
    ...
    def _persistence_rows(
        ...
        for position, (_, record) in enumerate(frame.iterrows()):
            ...
            if row["high"] < row["low"]:
                raise ValueError(
                    f"Invalid OHLC row for {row['symbol']} {row['date']}: high < low"
                )
            rows.append(row)
        return rows
    ...
    def _upsert_price_history(
        ...
        try:
            ...
            statement = pg_insert(PriceHistory).values(rows)
    ...
    def save_history(
        ...
        *,
        ...
        persistence_rows = self._persistence_rows(frame)
        counts = self._upsert_price_history(persistence_rows)
