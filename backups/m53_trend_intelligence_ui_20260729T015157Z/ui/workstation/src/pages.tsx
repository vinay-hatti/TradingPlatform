...
export function DailyScannerPage(){
  ...
  return <section>
    <div className="page-title"><div><h2>Daily scanner</h2><p>Ingest governed provider data when requested, then run analysis exclusively from persisted PostgreSQL data.</p></div><Badge value={selected?.status||'IDLE'}/></div>
...
export function MarketOverviewPage(){
  ...
  return <section className="market-overview-page">
    ...
    <Card title="Trend Intelligence">
      ...
      <div className="trend-intelligence-metrics"><Metric label="Bullish" value={tib.bullish??0} detail={`${fmt(tib.bullish_pct)}% of coverage`}/><Metric label="Neutral" value={tib.neutral??0}/><Metric label="Bearish" value={tib.bearish??0} detail={`${fmt(tib.bearish_pct)}% of coverage`}/><Metric label="Transition watch" value={tib.transition_watch_count??0}/><Metric label="Operational score" value={tio.score==null?'—':fmt(tio.score)} detail={tio.status||'NOT_AVAILABLE'}/></div>
      <div className="trend-intelligence-grid">
        <div><h4>Model distributions</h4><div className="distribution-list">{Object.entries(tid.base_trend||{}).map(([key,value]:any)=><span key={`base-${key}`}>{String(key).replaceAll('_',' ')}<b>{value}</b></span>)}{Object.entries(tid.transition||{}).map(([key,value]:any)=><span key={`transition-${key}`}>{String(key).replaceAll('_',' ')}<b>{value}</b></span>)}{Object.entries(tid.forecast||{}).map(([key,value]:any)=><span key={`forecast-${key}`}>{String(key).replaceAll('_',' ')}<b>{value}</b></span>)}</div></div>
    ...
    <div className="overview-two-column">
      <Card title="Trend operational governance"><div className="score-stack">{Object.entries(tio.assessments||{}).map(([name,value]:any)=><div key={name} className="operation-row"><span>{String(name).replaceAll('_',' ')}</span><Badge value={value.status||'NOT_AVAILABLE'}/><b>{fmt(value.score)}</b></div>)}</div><p className="overview-note">Milestone 52 closure eligible: <b>{tio.milestone_52_closure_eligible?'YES':'NO'}</b>. Calibration may remain NOT_ENOUGH_HISTORY without blocking governed use.</p></Card>
      <Card title="Institutional trend breadth"><div className="detail-list"><span>Participation breadth <b>{fmt(ti.institutional_overview?.participation_breadth_pct)}%</b></span><span>Leadership breadth <b>{fmt(ti.institutional_overview?.leadership_breadth_pct)}%</b></span><span>Deterioration watch <b>{fmt(ti.institutional_overview?.deterioration_watch_pct)}%</b></span><span>Average participation <b>{fmt(ti.institutional_overview?.average_participation_score)}</b></span><span>Average leadership <b>{fmt(ti.institutional_overview?.average_leadership_score)}</b></span><span>Average trend quality <b>{fmt(ti.institutional_overview?.average_trend_quality_score)}</b></span></div></Card>
    </div>
