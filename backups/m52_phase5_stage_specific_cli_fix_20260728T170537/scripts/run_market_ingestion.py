...
def _run_trend_intelligence_pipeline(args: argparse.Namespace, symbols: tuple[str, ...]) -> bool:
    ...
    if args.symbols or args.symbols_file:
        common.extend(["--symbols", ",".join(symbols)])
    if args.start:
        common.extend(["--start", args.start])

    # Phase 4 institutional population requires an explicit --end date. The unified
    # ingestion CLI allows --end to be omitted, so resolve it once here and propagate
    # the same governed as-of date to every Trend Intelligence stage.
    effective_end = args.end or date.today().isoformat()
    common.extend(["--end", effective_end])

    stages = (
        ("trend state", "scripts/run_trend_intelligence.py"),
        ("trend transitions", "scripts/run_trend_transition_intelligence.py"),
        ("trend forecasts", "scripts/run_trend_forecasting.py"),
        ("institutional participation", "scripts/run_institutional_trend_intelligence.py"),
    )
    for name, script in stages:
        _run_command_stage(name, [sys.executable, script, *common])
