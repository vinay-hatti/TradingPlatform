...
VERSION = "M77.19.7.3-POINT-IN-TIME-SYMBOL-SPECIFIC-STOCK-INTELLIGENCE-REPLAY-1.0"
...
def maybe_resume(
    replay_root: Path, symbol_row: Mapping[str, Any], cadence: str, upstream_sha: str
) -> dict[str, Any] | None:
...
def replay_one_symbol_cadence(
    ...
    for as_of in observation_dates:
        ...
        direction = lookup(mapping, "direction")
        score = lookup(mapping, "overall_score", "score")
        confidence = lookup(mapping, "confidence", "confidence_score")
        if direction is None or score is None or confidence is None:
            raise ReplayError(f"{symbol} {as_of}: native profile missing direction/score/confidence")
        record = {
            "symbol": symbol,
            "as_of": as_of.isoformat(),
            "cadence": cadence,
...
def main() -> int:
    ...
    ap.add_argument(
        "--output-root",
        default="research_data/m77_19_7_3/point_in_time_stock_intelligence_replay",
    )
    ap.add_argument(
        "--output-json",
        default="reports/m77_19_7_3_point_in_time_symbol_specific_stock_intelligence_replay.json",
    )
    ap.add_argument(
        "--output-csv",
        default="reports/m77_19_7_3_point_in_time_symbol_specific_stock_intelligence_replay.csv",
    )
    ...
    for row in certified_rows:
        for cadence in cadences:
            if args.resume:
                prior = maybe_resume(replay_root, row, cadence, upstream_sha)
    ...
    print("=== M77.19.7.3 POINT-IN-TIME SYMBOL-SPECIFIC STOCK INTELLIGENCE REPLAY ===")
    print("status:", status)
