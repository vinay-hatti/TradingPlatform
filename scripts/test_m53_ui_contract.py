...
def main() -> None:
    ...
    assert 'Trend operational governance' not in pages
    assert 'DistributionGroup title="Base trend"' in pages
    assert 'DistributionGroup title="Institutional participation"' in pages
    assert pages.index('Card title="Institutional trend breadth"') < pages.index('Card title="Trend Intelligence"')
