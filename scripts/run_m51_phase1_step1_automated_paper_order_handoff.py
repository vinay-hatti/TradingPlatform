...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Milestone 51 Phase 1 Step 1 automated IBKR paper-order handoff."
    )
    ...
    parser.add_argument(
        "--output-json",
        default="reports/m51/phase1/step1/automated_paper_order_handoff.json",
    )
...
def main() -> None:
    ...
    candidate = AutomatedPaperOrderCandidate.from_dict(payload)

    transport = None
    broker_service = None
    try:
        if args.mode == "SUBMIT":
            binding = load_binding(candidate.portfolio_id)
        ...
        service = AutomatedPaperOrderHandoffService(
            SessionLocal, broker_order_service=broker_service
        )
        ...
        if result.status == "REJECTED_BY_HANDOFF_POLICY":
            raise SystemExit(2)
    finally:
