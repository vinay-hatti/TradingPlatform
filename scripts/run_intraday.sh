...
export JOB_NAME="intraday_underlying_options_refresh"
export LOCK_NAME="m69_6_market_pipeline"
export LOG_FILE="/Users/vinay.hatti/TradingPlatform/logs/m69_6_intraday_options.log"
...
EXPECTED_STOCK_RUN_ID="$("${UV_BIN}" run python scripts/verify_intraday_ingestion_lineage.py --capture-current-stock-run)"
if [[ -z "${EXPECTED_STOCK_RUN_ID}" ]]; then
  echo "[$(date)] FAILED job=${JOB_NAME} reason=NO_CURRENT_STOCK_LINEAGE_AFTER_UNDERLYING" | tee -a "${LOG_FILE}"
  exit 31
fi
echo "[$(date)] GOVERNED_LINEAGE expected_stock_scanner_run_id=${EXPECTED_STOCK_RUN_ID}" | tee -a "${LOG_FILE}"
run_cmd "${UV_BIN}" run python scripts/ingest_options_data.py "${OPTIONS_ARGS[@]}"
run_cmd "${UV_BIN}" run python scripts/verify_intraday_ingestion_lineage.py --expected-stock-run-id "${EXPECTED_STOCK_RUN_ID}"
run_cmd "${UV_BIN}" run python scripts/run_intraday_active_universe_shadow.py intraday
echo "[$(date)] END job=${JOB_NAME} status=READY stock_scanner_run_id=${EXPECTED_STOCK_RUN_ID}" | tee -a "${LOG_FILE}"
