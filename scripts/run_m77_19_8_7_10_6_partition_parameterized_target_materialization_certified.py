...
def main():
    ...
    ap.add_argument("--active-partition-label",choices=("VALIDATION","FINAL_HOLDOUT"),default="VALIDATION")
    ap.add_argument("--partition-start",default=VALIDATION_START)
    ap.add_argument("--partition-end",default=VALIDATION_END)
    ap.add_argument("--expected-feature-rows",type=int,default=EXPECTED_FEATURE_ROWS)
    ap.add_argument("--expected-feature-symbols",type=int,default=EXPECTED_FEATURE_SYMBOLS)
    ...
    if args.active_partition_label=="VALIDATION":
        ...
        if vb.get("validation_row_count")!=args.expected_feature_rows or vb.get("validation_symbol_count")!=args.expected_feature_symbols:
        ...
        if vb.get("status")!="READY" or vb.get("final_holdout_feature_matrix_materialized") is not True:
            raise AuthorityError("Final Holdout feature matrix not READY/certified")
        if vb.get("backfill_feature_row_count")!=args.expected_feature_rows or vb.get("backfill_feature_symbol_count")!=args.expected_feature_symbols:
            raise AuthorityError("Final Holdout feature cardinality changed")
        if vb.get("final_holdout_targets_opened") is not False or vb.get("final_holdout_scoring_performed") is not False:
            raise AuthorityError("Final Holdout target/scoring governance violation")
    ...
    if val_symbol_files!=args.expected_feature_symbols:
        raise AuthorityError(f"active feature root changed: {val_symbol_files} files")
    ...
    for vf in sorted(val_root.glob("*.jsonl.gz")):
        ...
        for row in iter_jsonl(vf):
            ...
            if not (args.partition_start<=as_of<=args.partition_end):
                raise AuthorityError(f"{symbol} {as_of}: active feature outside authorized window")
            if as_of not in idx:
                for h in HORIZONS:
            ...
            for h in HORIZONS:
                ...
                if j>=len(dates):
                    ...
                    continue
                td=dates[j]
                # Purge any label whose target session exits the frozen active partition.
                if td>args.partition_end:
                ...
                target={
                    ...
                    "partition":args.active_partition_label,
    ...
    if total_feature_rows!=args.expected_feature_rows:
        raise AuthorityError(f"active feature row count changed during target materialization: {total_feature_rows}")
    ...
    report={
        ...
        "active_partition_label":args.active_partition_label,
        "active_partition_start":args.partition_start,
        "active_partition_end":args.partition_end,
