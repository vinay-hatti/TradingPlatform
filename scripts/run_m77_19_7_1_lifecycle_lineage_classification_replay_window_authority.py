...
VERSION = "M77.19.7.1-LIFECYCLE-LINEAGE-CLASSIFICATION-REPLAY-WINDOW-AUTHORITY-1.0"
...
def fetch_post_anchor_history(
    ...
    for bar in results:
        ...
        if ts is None:
            continue
        d = date.fromtimestamp(ts / 1000.0).isoformat()
        if d >= anchor_date:
...
def main() -> int:
    ...
    parser.add_argument(
        "--output-json",
        default="reports/m77_19_7_1_lifecycle_lineage_classification_replay_window_authority.json",
    )
    parser.add_argument(
        "--output-csv",
        default="reports/m77_19_7_1_lifecycle_lineage_classification_replay_window_authority.csv",
    )
    ...
    print("=== M77.19.7.1 LIFECYCLE / LINEAGE CLASSIFICATION & REPLAY WINDOW AUTHORITY ===")
