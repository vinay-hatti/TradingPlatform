...
def backup(path):
    target=path.with_suffix(path.suffix+".m33_phase8.bak")
    if not target.exists():
        shutil.copy2(path,target)

def main():
    text=APP.read_text(encoding="utf-8")
    imp="from trading_ai.ui.api.security_compliance_center import router as security_compliance_center_router\n"
    if imp not in text:
        anchors=[
    ...
    if "security_compliance_center_router," not in text:
        anchors=[
        ...
        text=text.replace(marker,marker+"  security_compliance_center_router,\n")
    text=text.replace('version="33.7.0"','version="33.8.0"')
    backup(APP)
    APP.write_text(text,encoding="utf-8")

    html=INDEX.read_text(encoding="utf-8")
    js='<script src="/static/security_compliance_center.js"></script>'
    if js not in html:
        html=html.replace("</body>",f"  {js}\n</body>")
    backup(INDEX)
    INDEX.write_text(html,encoding="utf-8")
    print("Milestone 33 Phase 8 integration completed.")
