...
for line in rows:
    try:
        x=json.loads(line)
        if x.get("version") in {"INTRADAY-ACTIVE-UNIVERSE-SHADOW-1.0","INTRADAY-ACTIVE-UNIVERSE-SHADOW-1.1"}:
            bad.append(line)
        else: keep.append(line)
    except Exception: keep.append(line)
backup=p.with_suffix(".pre_1_2.jsonl")
shutil.copy2(p,backup)
p.write_text(("\n".join(keep)+"\n") if keep else "")
if latest.exists():
    try:
        x=json.loads(latest.read_text())
        if x.get("version") in {"INTRADAY-ACTIVE-UNIVERSE-SHADOW-1.0","INTRADAY-ACTIVE-UNIVERSE-SHADOW-1.1"}: latest.unlink()
