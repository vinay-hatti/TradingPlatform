...
class PreferredOpportunityRequest(BaseModel):
...
@router.post("/{opportunity_id}/preferred", response_model=ApiEnvelope)
def designate_preferred(opportunity_id: str, payload: PreferredOpportunityRequest, request: Request, actor: str = Depends(require_mutation_access)):
    with SessionLocal() as session:
        try:
            record = OpportunityService(session).designate_preferred(
                opportunity_id, expected_version=payload.expected_version, actor=actor, reason=payload.reason
            )
