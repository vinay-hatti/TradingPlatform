...
BACKUP="$ROOT/.rollback/m53_phase6_ai_workspace_productivity_$(date +%Y%m%d_%H%M%S)"
FILES=(
  ...
  "scripts/test_m53_phase6_ai_workspace_productivity.py"
)
mkdir -p "$BACKUP"
for rel in "${FILES[@]}"; do
...
printf '%s\n' "$BACKUP" > "$ROOT/.m53_phase6_last_backup"
echo "Applied Milestone 53 Phase 6. Backup: $BACKUP"
