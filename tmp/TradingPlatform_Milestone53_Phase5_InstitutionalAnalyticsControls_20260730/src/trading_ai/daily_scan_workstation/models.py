...
class DailyScanRequest(BaseModel):
    ...
    direction: Literal["both", "bullish", "bearish"] = "both"
    minimum_trend_quality_score: float = Field(default=0.0, ge=0, le=100)
    minimum_trend_alignment_score: float = Field(default=0.0, ge=0, le=100)
    minimum_trend_confidence: float = Field(default=0.0, ge=0, le=100)
    allowed_trend_stages: list[str] = Field(default_factory=list, max_length=10)
    minimum_transition_confirmation_score: float = Field(default=0.0, ge=0, le=100)
    maximum_reversal_risk_score: float = Field(default=100.0, ge=0, le=100)
    maximum_exhaustion_risk_score: float = Field(default=100.0, ge=0, le=100)
    require_breakout: bool = False
    require_fresh_dealer_context: bool = False
    minimum_dealer_score_adjustment: float = Field(default=-100.0, ge=-100, le=100)
    minimum_market_structure_confidence: float = Field(default=0.0, ge=0, le=100)
    minimum_participation_score: float = Field(default=0.0, ge=0, le=100)
    minimum_leadership_score: float = Field(default=0.0, ge=0, le=100)
    minimum_institutional_conviction_score: float = Field(default=0.0, ge=0, le=100)
    maximum_deterioration_risk_score: float = Field(default=100.0, ge=0, le=100)
    minimum_breadth_confirmation_score: float = Field(default=0.0, ge=0, le=100)
    minimum_cross_asset_confirmation_score: float = Field(default=0.0, ge=0, le=100)
