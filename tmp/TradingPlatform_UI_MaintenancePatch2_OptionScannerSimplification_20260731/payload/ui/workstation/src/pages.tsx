...
function persistedSnapshotStatus(run:any):PersistedSnapshotStatus{
  const rawTimestamp=run?.completed_at||run?.updated_at||run?.created_at||null;
  const timestamp=rawTimestamp?String(rawTimestamp):null;
  let ageLabel='Unknown';
  let freshness:PersistedSnapshotStatus['freshness']='UNKNOWN';
  if(timestamp){
    const ageMs=Math.max(0,Date.now()-new Date(timestamp).getTime());
    const ageMinutes=Math.floor(ageMs/60000);
    if(ageMinutes<60){ageLabel=`${ageMinutes} min`;freshness='FRESH'}
    else if(ageMinutes<360){ageLabel=`${Math.floor(ageMinutes/60)} hr`;freshness='AGING'}
    else{ageLabel=`${Math.floor(ageMinutes/1440)} day`;freshness='STALE'}
  }
  ...
  return {
    ...
    ageLabel,
    freshness,
    coverage:String(summary.coverage??summary.coverage_pct??'—'),
    status:String(run?.status||'UNKNOWN'),
    symbolCount:String(summary.symbol_count??summary.processed_symbols??summary.universe_symbols??'—'),
    optionContracts:String(summary.option_contract_count??summary.options_contracts??summary.contract_count??'—'),
  };
}
...
function ScannerWorkspacePage({config}:{config:ScannerWorkspaceConfig}){
  ...
  const candidates=results?.recommendations?.candidates||[];const trades=results?.trades?.trades||[];const latestIngestion=runs.find((r:any)=>r.kind==='DATA_REFRESH'&&r.status==='SUCCEEDED')||runs.find((r:any)=>r.kind==='DATA_REFRESH');const snapshot=persistedSnapshotStatus(latestIngestion);
  ...
  return <section className={isOptionWorkspace?'option-scanner-workspace':''}>
    ...
    {isOptionWorkspace&&<>
      <div className={`snapshot-status snapshot-${snapshot.freshness.toLowerCase()}`}>
        <div className="snapshot-status-title"><span className="eyebrow">Published persisted snapshot</span><strong>{snapshot.timestamp?new Date(snapshot.timestamp).toLocaleString():'No published snapshot found'}</strong><small>Option Scanner is read-only and never invokes Yahoo, Polygon, or market ingestion.</small></div>
        <div><span>Freshness</span><strong>{snapshot.freshness}</strong><small>{snapshot.ageLabel} old</small></div>
        <div><span>Publication</span><strong>{snapshot.status}</strong><small>Latest persisted data run</small></div>
        <div><span>Coverage</span><strong>{snapshot.coverage}</strong><small>{snapshot.symbolCount} symbols</small></div>
        <div><span>Options</span><strong>{snapshot.optionContracts}</strong><small>Persisted contracts</small></div>
      </div>
