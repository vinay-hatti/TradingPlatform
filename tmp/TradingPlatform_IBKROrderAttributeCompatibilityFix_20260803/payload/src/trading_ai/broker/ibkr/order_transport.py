...
class IbapiPaperOrderTransport:
    ...
    def submit_combo_order(self, request: IbkrPaperComboOrderRequest) -> int:
        ...
        app.begin_order_ack(oid)
        LOGGER.info("IBKR placeOrder sending atomic combo order_id=%s aggregate_id=%s legs=%s", oid, request.aggregate_id, len(request.combo_legs))
