...
class ExecutionWorkspaceService:
 ...
 def submit(self,id,expected_version,actor,reason,confirmation):
  ...
  if len(m.legs_json)!=1:raise ValueError('Direct IBKR submission currently supports one option leg per intent; multi-leg plans must be split or broker combo support enabled')
  ...
  leg=m.legs_json[0];aggregate_id=f'M59-{m.execution_intent_id}';client_id=f'M59-CLIENT-{m.execution_intent_id}'
  canonical=self.s.get(CanonicalOrderModel,aggregate_id)
  if canonical is None:
   ts=now();qty=float(leg['quantity']);canonical=CanonicalOrderModel(aggregate_id=aggregate_id,client_order_id=client_id,account_id=m.portfolio_id,idempotency_key=aggregate_id,order_type='LMT',time_in_force='DAY',state='VALIDATED',version=1,total_quantity=qty,filled_quantity=0,remaining_quantity=qty,average_fill_price=None,limit_price=float(leg['limit_price']),stop_price=None,outside_regular_hours=False,strategy_name=m.strategy,broker_order_id=None,parent_aggregate_id=None,root_aggregate_id=aggregate_id,replace_count=0,legs_json=m.legs_json,created_at=ts,updated_at=ts,terminal_at=None,last_event_id=None,metadata_json={'execution_intent_id':m.execution_intent_id,'paper_only':True});self.s.add(canonical);self.s.flush()
  self.s.commit()
  request=IbkrPaperOrderRequest(aggregate_id=aggregate_id,client_order_id=client_id,portfolio_id=m.portfolio_id,broker_account_id=binding.broker_account_id,symbol=m.symbol,security_type='OPT',side=str(leg['side']),quantity=float(leg['quantity']),order_type='LMT',time_in_force='DAY',limit_price=float(leg['limit_price']),currency=binding.base_currency or 'USD',exchange='SMART',local_symbol=str(leg.get('option_symbol') or ''),expiry=str(leg.get('expiry') or '').replace('-',''),strike=float(leg.get('strike') or 0),right='C' if str(leg.get('option_right')).upper()=='CALL' else 'P',multiplier='100',transmit=True,metadata={'execution_intent_id':m.execution_intent_id,'paper_only':True})
  ...
  previous=m.state;m.version+=1;m.state='SUBMITTED';m.submitted_at=now();m.updated_at=m.submitted_at;m.broker_json={**m.broker_json,'connection':connection,'submission':result,'aggregate_id':aggregate_id};self._audit(m,previous,m.state,'PAPER_ORDER_SUBMITTED',actor,reason,{'broker':result});self.s.commit();return self.dto(m)
