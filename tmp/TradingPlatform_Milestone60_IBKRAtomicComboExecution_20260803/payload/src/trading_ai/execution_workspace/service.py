...
class ExecutionWorkspaceService:
 ...
 def create_from_trade_plan(self,trade_plan_id:str,actor:str,portfolio_id:str|None=None):
  ...
  actions={str(x.get('side','')).upper() for x in legs};described=all(x.get('expiry') and x.get('strike') is not None and x.get('option_right') and x.get('limit_price') is not None and float(x.get('quantity',0))>0 for x in legs);combo_supported=len(legs)==1 or (2<=len(legs)<=4 and actions=={'BUY','SELL'} and described)
 ...
 def submit(self,id,expected_version,actor,reason,confirmation):
  ...
  legs=list(m.legs_json);aggregate_id=f'M59-{m.execution_intent_id}';client_id=f'M59-CLIENT-{m.execution_intent_id}'
  ...
  previous=m.state;m.version+=1;m.state='SUBMITTED';m.submitted_at=now();m.updated_at=m.submitted_at;m.broker_json={**m.broker_json,'connection':connection,'submission':result,'aggregate_id':aggregate_id,'atomic_combo':len(legs)>1,'resolved_combo_legs':[x.__dict__ for x in resolved_legs]};self._audit(m,previous,m.state,'PAPER_ORDER_SUBMITTED',actor,reason,{'broker':result,'atomic_combo':len(legs)>1});self.s.commit();return self.dto(m)
