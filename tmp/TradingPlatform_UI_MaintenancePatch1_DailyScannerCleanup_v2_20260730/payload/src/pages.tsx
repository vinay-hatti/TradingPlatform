...
function ScannerWorkspacePage({config}:{config:ScannerWorkspaceConfig}){
  ...
  return <section className={isOptionWorkspace?'option-scanner-workspace':''}>
    ...
    {isOptionWorkspace&&<Card title="Workspace productivity">
      <div className="option-productivity-grid">
    ...
    {isOptionWorkspace&&<>
      ...
      <div className="strategy-selector-panel">
        <div className="control-section-heading"><div><span>01</span><h3>Strategy intent</h3></div><p>Selecting a strategy applies governed universe, ranking, horizon, liquidity, spread, and diversification defaults. Every value remains visible and editable.</p></div>
        <div className="strategy-card-grid">{OPTION_SCANNER_STRATEGIES.map(item=><button key={item.id} type="button" className={strategy===item.id?'strategy-card active':'strategy-card'} onClick={()=>applyStrategy(item.id)}><strong>{item.label}</strong><span>{item.description}</span></button>)}</div>
        <div className="strategy-profile-row"><label>Risk profile<select value={riskProfile} onChange={e=>setRiskProfile(e.target.value as OptionScannerRiskProfile)}>{Object.entries(OPTION_SCANNER_RISK_PROFILES).map(([id,item])=><option key={id} value={id}>{item.label}</option>)}</select></label><div><span>Applied configuration</span><strong>{minimumScore}+ score · {minimumDte}–{maximumDte} DTE · OI {minimumOpenInterest}+ · Volume {minimumOptionVolume}+ · Spread ≤ {(maximumSpreadPct*100).toFixed(0)}%</strong></div></div>
      </div>
