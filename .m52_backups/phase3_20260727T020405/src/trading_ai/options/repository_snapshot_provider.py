...
class RepositoryOptionSnapshotProvider:
    ...
    def chain(self, underlying, *, signal, target_expiration, target_strike, as_of,
        ...
        for row in rows:
            ...
            dte = (expiry - date.fromisoformat(str(row.get("quote_date"))[:10])).days
        ...
        if not contracts:
            raise LiveOptionDataError(
                ...
                f"{self.snapshot_as_of or as_of}. The Daily Scanner is read-only; "
                "run the governed options-ingestion workflow separately if data is absent."
            )
        return contracts
