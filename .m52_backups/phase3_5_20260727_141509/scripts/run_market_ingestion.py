...
def main(argv=None) -> int:
    ...
    args = _resolve_force_controls(build_parser().parse_args(argv))
    ...
    if args.data_scope in {"options", "all"}:
        ...
        try:
            profile = OptionHistoryIngestionService(session, provider, manifest_store=manifest).run(
                ...
                resume=not args.force_options_refresh,
        ...
        options_refreshed = bool(profile.inserted_records or profile.updated_records)
        print(
    ...
    if not args.skip_market_overview:
        try:
            _run_market_overview(
                args,
                upstream_refreshed=bool(options_refreshed or dealer_refreshed or args.force_underlying_refresh),
            )
