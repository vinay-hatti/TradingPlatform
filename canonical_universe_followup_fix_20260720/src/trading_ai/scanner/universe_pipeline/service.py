        ...
        *,
        ...
        try:
            if not report_only and not skip_universe_refresh and not resume_completed(PipelineStage.BUILDING_UNIVERSE):
                ...
                stage = self._stage(PipelineStage.BUILDING_UNIVERSE, build_universe); stages.append(stage); universe_count = int(stage.details.get("symbol_count", 0)); checkpoint(PipelineStage.BUILDING_UNIVERSE)
            else:
                if not canonical_csv.is_file():
                    raise FileNotFoundError(f"Canonical universe not found: {canonical_csv}")
                from trading_ai.universe import CanonicalUniverse
                universe_count = len(CanonicalUniverse(canonical_csv).symbols())
            ...
            def validate_publication():
                ...
                # In canonical-only mode the CSV is an operator-controlled allowlist and is
                # intentionally not rebuilt by this run.  A stale universe-refresh manifest
                # must therefore not invalidate the authoritative CSV.
                universe_expected = "" if skip_universe_refresh else str(self._read_json(universe_manifest).get("csv_sha256", ""))
                ...
                invalid = [item.name for item in items if not item.exists or (self.policy.require_checksum_validation and not item.checksum_valid)]
                if invalid: raise RuntimeError("Publication validation failed: " + ", ".join(invalid))
