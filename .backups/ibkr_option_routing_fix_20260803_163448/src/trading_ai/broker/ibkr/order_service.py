    ...
    def submit_combo(self, request: IbkrPaperComboOrderRequest) -> dict:
        ...
        try:
            ...
            row = BrokerOrderModel(
                ...
                stop_price=None,
                status=str(acknowledgement.get("status") or "AWAITING_BROKER_ACK").upper(),
                ...
                raw_json={"request": raw_request, "response": acknowledgement, "error": ({"stage":"BROKER_ACKNOWLEDGEMENT","code":acknowledgement.get("error_code"),"message":str(acknowledgement.get("error_message") or ""),"retryable":False} if str(acknowledgement.get("status") or "").upper() in {"REJECTED","INACTIVE"} else None), "paper_only": True, "atomic_combo": True, "broker_acknowledgement": acknowledgement},
            )
            s.add(row)
            canonical.broker_order_id = str(broker_order_id)
            canonical.state = "REJECTED" if str(acknowledgement.get("status") or "").upper() in {"REJECTED","INACTIVE"} else "SUBMITTED"
            canonical.updated_at = now
            canonical.terminal_at = now if canonical.state == "REJECTED" else None
