...
export function DailyScannerPage(){
  ...
  const candidates=results?.recommendations?.candidates||[];const trades=results?.trades?.trades||[];const latestIngestion=runs.find((r:any)=>r.kind==='DATA_REFRESH');const selectedUniverse=universes.find((u:any)=>u.id===universe);const trendEmptyMessage=selected?.status==='FAILED'&&selectedUniverse?.dynamic?(selected?.stderr||selectedUniverse?.message||'').trim():'';
  ...
  return <section>
    ...
    {trendEmptyMessage&&<Card title="Universe resolution"><p className="empty">{trendEmptyMessage}</p></Card>}
