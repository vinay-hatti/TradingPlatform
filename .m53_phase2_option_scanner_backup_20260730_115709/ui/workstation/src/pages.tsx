...
function ScannerWorkspacePage({config}:{config:ScannerWorkspaceConfig}){
  ...
  useEffect(()=>{
    ...
    localStorage.setItem(`trading-ai:${config.workspaceKey}:scan-controls`,JSON.stringify({universe,symbols,minimumScore,top,expirationMode,minimumDte,maximumDte,maxPerExpiry,refreshMode,ingestionScope,autoRefresh,minimumCoverage,maximumFailedSymbols,continueOnDegraded}));
  },[preferencesReady,config.workspaceKey,universe,symbols,minimumScore,top,expirationMode,minimumDte,maximumDte,maxPerExpiry,refreshMode,ingestionScope,autoRefresh,minimumCoverage,maximumFailedSymbols,continueOnDegraded]);
...
const OPTION_SCANNER_WORKSPACE: ScannerWorkspaceConfig = {
  ...
  actionLabel: 'Run database-only option scan',
};
