...
class InstitutionalTrendRepository:
    ...
    def latest(self, symbol: str, reference_date=None):
        ...
        with self.session_factory() as session:
            row = session.execute(
                text(
                    ...
                    WHERE symbol=:symbol{reference_clause}
    ...
    def scanner_context(self, symbol: str, maximum_age_days: int = 3, reference_date=None):
        ...
        payload = self.latest(symbol, reference_date=effective_reference_date)
        if not payload:
            return neutral
        ...
        if age > maximum_age_days:
            neutral.update(
                ...
                institutional_snapshot_date=snapshot_date.isoformat(),
                institutional_snapshot_age_days=age,
                institutional_context_warning="Institutional trend snapshot exceeds governed maximum business-day age.",
            )
            return neutral
        neutral.update(
