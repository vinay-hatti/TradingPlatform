        ...
        try:
            ...
            for status in self.transport.order_statuses(binding.broker_account_id):
                ...
                if canonical is not None:
                    canonical.state=self._canonical_state(status.status); canonical.filled_quantity=status.filled_quantity; canonical.remaining_quantity=status.remaining_quantity; canonical.average_fill_price=status.average_fill_price or None; canonical.broker_order_id=str(status.broker_order_id); canonical.updated_at=status.updated_at
                updated += 1
