...
from trading_ai.research.m77.mean_reversion_extension_state import (
    STATES, MeanReversionExtensionConfig, MeanReversionExtensionError,
...
def _authority():
    ...
    a["dist_sma_20"]=-.10+.02*i
    a["dist_sma_60"]=-.12+.025*i
    a["dist_ema_21"]=-.09+.018*i
    a["dist_ema_50"]=-.11+.022*i
    a["dist_prev_high_20"]=-.20+.02*i
    a["dist_prev_low_20"]=.20-.02*i
    a["dist_prev_high_60"]=-.25+.025*i
    a["dist_prev_low_60"]=.25-.025*i
    return a
...
def test_feature_join_complete(tmp_path):
    ...
    p,m=load_panel(MeanReversionExtensionConfig(
...
def test_post_2017_rejected(tmp_path):
    ...
    a=_authority();ap=tmp_path/"panel.pkl.gz";a.to_pickle(ap,compression="gzip",protocol=5)
    with pytest.raises(MeanReversionExtensionError):
        load_panel(MeanReversionExtensionConfig(
            project_root=str(tmp_path),executable_panel_path=str(ep),feature_authority_path=str(ap)
        ))
...
def test_all_frozen_states_materialize(tmp_path):
    e=_exec_panel();ep=tmp_path/"exec.csv.gz";e.to_csv(ep,index=False,compression="gzip")
    a=_authority();ap=tmp_path/"panel.pkl.gz";a.to_pickle(ap,compression="gzip",protocol=5)
    p,_=load_panel(MeanReversionExtensionConfig(
        project_root=str(tmp_path),executable_panel_path=str(ep),feature_authority_path=str(ap)
    ))
    for state in STATES:
...
def test_readiness_explicit():
    ...
    assert bool(r.iloc[0]["development_ready_extension_state"])

def test_missing_feature_parity_fails_closed(tmp_path):
    e=_exec_panel();ep=tmp_path/"exec.csv.gz";e.to_csv(ep,index=False,compression="gzip")
    a=_authority();a.loc[0,"dist_sma_20"]=np.nan
    ap=tmp_path/"panel.pkl.gz";a.to_pickle(ap,compression="gzip",protocol=5)
    with pytest.raises(MeanReversionExtensionError,match="feature parity failed"):
        load_panel(MeanReversionExtensionConfig(
            project_root=str(tmp_path),executable_panel_path=str(ep),feature_authority_path=str(ap)
        ))
