...
def test_stale_retirement_is_bounded_idempotent_and_preserves_history(factory):
    with factory() as session:
        for index in range(25):
            risk_id = f"M64-RISK-HISTORICAL-{index:03d}"
            payload = _portfolio_decision(risk_id)
            payload["decision_identity"] = {
                **payload["decision_identity"],
                "opportunity_id": f"m62-opp-historical-{index:03d}",
            }
            payload["audit_marker"] = index
            session.add(PortfolioDecisionIntelligenceModel(
                decision_intelligence_id=f"M64-DI-HISTORICAL-{index:03d}",
                portfolio_id=PORTFOLIO,
                opportunity_id=f"m62-opp-historical-{index:03d}",
                institutional_decision_snapshot_id=f"m62-decision-historical-{index:03d}",
                risk_snapshot_id=risk_id,
                portfolio_fit_score=70,
                opportunity_cost_score=70,
                final_portfolio_score=70,
                recommended_quantity=1,
                recommended_capital=500,
                decision="ACCEPT",
                rank=index + 2,
    ...
    service = InstitutionalDecisionIntelligenceService(factory)
    service.STALE_RETIREMENT_BATCH_SIZE = 7
    events: list[tuple[str, dict]] = []
    with factory() as session:
        retired = service._retire_stale_decisions(
            session,
            portfolio_id=PORTFOLIO,
            current_risk_snapshot_id=NEW_RISK,
            current_stock_run_id=RUN,
            progress=lambda stage, details: events.append((stage, details)),
        )
        session.commit()

    # The fixture's prior OLD_RISK decision plus 25 historical rows are retired.
    assert retired == 26
    batch_events = [details for stage, details in events if stage == "stale_decision_retirement_batch"]
    assert len(batch_events) == 4
    assert max(item["batch_rows"] for item in batch_events) <= 7
    assert batch_events[-1]["superseded"] == 26

    with factory() as session:
        historical = list(session.scalars(select(PortfolioDecisionIntelligenceModel).where(
            PortfolioDecisionIntelligenceModel.risk_snapshot_id.like("M64-RISK-HISTORICAL-%")
        )).all())
        assert len(historical) == 25
        assert all(row.payload_json["lifecycle"]["status"] == "SUPERSEDED" for row in historical)
        assert [row.payload_json["audit_marker"] for row in historical] == list(range(25))

        second_retirement = service._retire_stale_decisions(
            session,
            portfolio_id=PORTFOLIO,
            current_risk_snapshot_id=NEW_RISK,
            current_stock_run_id=RUN,
