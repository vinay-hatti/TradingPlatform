...
class StockOpportunityScoringEngine:
    def score(self,p:StockIntelligenceProfile,freshness=100):
        ...
        part=p.participation.score if p.participation else 50; br=p.breakout.confirmation if p.breakout else 0; ctx=p.context.score if p.context else 50; align=p.alignment_score; volume=(p.institutional_volume.institutional_participation_score if p.institutional_volume else 50)
        structure=p.structure; weights={'direction':.23,'alignment':.18,'participation':.12,'volume':.12,'breakout':.14,'context':.13,'levels':.08}
        if structure in ('SIDEWAYS','COMPRESSION'):weights={'direction':.10,'alignment':.11,'participation':.14,'volume':.16,'breakout':.11,'context':.10,'levels':.28}
        ...
        overall=bull*weights['direction']+align*weights['alignment']+part*weights['participation']+volume*weights['volume']+br*weights['breakout']+ctx*weights['context']+level_score*weights['levels']; overall*=freshness/100
