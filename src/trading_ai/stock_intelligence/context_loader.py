...
class PersistedStockContextLoader:
    ...
    def forecast(self, symbol: str) -> SimpleNamespace | None:
        ...
        return _namespace(
            ...
            bullish_probability=float(payload.get("bullish_probability") or 0.0),
            bearish_probability=float(payload.get("bearish_probability") or 0.0),
            continuation_probability=float(payload.get("continuation_probability") or 0.0),
            reversal_probability=float(payload.get("reversal_probability") or 0.0),
            expected_return_pct=float(payload.get("expected_return_pct") or 0.0),
            expected_volatility_pct=float(payload.get("expected_volatility_pct") or 0.0),
            directional_consistency=bool((payload.get("metadata") or {}).get("directional_consistency", True)),
            conflict_codes=tuple((payload.get("metadata") or {}).get("conflict_codes") or ()),
            prevailing_trend_direction=str((payload.get("metadata") or {}).get("prevailing_trend_direction") or "UNKNOWN").upper(),
