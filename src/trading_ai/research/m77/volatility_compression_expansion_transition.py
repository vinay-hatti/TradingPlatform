...
VERSION="M77.32.0-VOLATILITY-COMPRESSION-EXPANSION-TRANSITION-EDGE-DISCOVERY-1.0"
...
TRANSITION_STATES=(
    "RV10_TO_RV60_COMPRESSION_BOTTOM20",
    "RV10_TO_RV60_EXPANSION_TOP20",
    "RV10_TO_RV20_DECELERATION_BOTTOM20",
    "RV10_TO_RV20_ACCELERATION_TOP20",
    "RANGE_ATR_COMPRESSION_BOTTOM20",
    "RANGE_ATR_EXPANSION_TOP20",
...
class VolatilityTransitionConfig:
    project_root:str
    ...
    output_dir:str="research_data/m77_32/volatility_compression_expansion_transition_edge_discovery"
...
def load_panel(cfg:VolatilityTransitionConfig)->tuple[pd.DataFrame,dict[str,Any]]:
    ...
    if not ep.exists():raise VolatilityTransitionError(f"Executable Development panel missing: {ep}")
    if not ap.exists():raise VolatilityTransitionError(f"Frozen M77.21 feature authority missing: {ap}")
    ...
    if p.empty:raise VolatilityTransitionError("Frozen executable cohort missing")
    if (p["as_of"]>DEVELOPMENT_END).any():
        raise VolatilityTransitionError("M77.32 refuses post-2017 executable evidence")

    auth=pd.read_pickle(ap,compression="gzip")
    auth=auth.copy()
    if "symbol" not in auth.columns or "as_of" not in auth.columns:
        raise VolatilityTransitionError("M77.21 feature authority missing symbol/as_of")
    auth["symbol"]=auth["symbol"].astype(str).str.upper()
    auth["as_of"]=pd.to_datetime(auth["as_of"],errors="coerce")

    features=("rv_10","rv_20","rv_60","range_atr")
    missing=[c for c in features if c not in auth.columns]
    if missing:
        raise VolatilityTransitionError(f"M77.21 feature authority missing required volatility features: {missing}")
    auth=auth[["symbol","as_of",*features]].copy()
    if auth.duplicated(["symbol","as_of"],keep=False).any():
        raise VolatilityTransitionError("M77.21 feature authority is not unique on (symbol, as_of)")
    ...
    if (~complete).any():
        ...
        raise VolatilityTransitionError(
            f"M77.32 point-in-time feature parity failed; missing_rows={int((~complete).sum())}, sample={sample}"
        )
    ...
    eps=1e-12
    p["rv10_to_rv60"]=p["rv_10"]/(p["rv_60"].abs()+eps)
    p["rv10_to_rv20"]=p["rv_10"]/(p["rv_20"].abs()+eps)

    # Same-date point-in-time cross-sectional transition ranks.
    p["rv10_rv60_rank"]=p.groupby("as_of")["rv10_to_rv60"].rank(pct=True,method="average")
    p["rv10_rv20_rank"]=p.groupby("as_of")["rv10_to_rv20"].rank(pct=True,method="average")
    p["range_atr_rank"]=p.groupby("as_of")["range_atr"].rank(pct=True,method="average")
...
def _state_mask(p:pd.DataFrame,state:str)->pd.Series:
    if state=="RV10_TO_RV60_COMPRESSION_BOTTOM20":
        return p["rv10_rv60_rank"]<=BOTTOM_Q
    if state=="RV10_TO_RV60_EXPANSION_TOP20":
        return p["rv10_rv60_rank"]>=TOP_Q
    if state=="RV10_TO_RV20_DECELERATION_BOTTOM20":
        return p["rv10_rv20_rank"]<=BOTTOM_Q
    if state=="RV10_TO_RV20_ACCELERATION_TOP20":
        return p["rv10_rv20_rank"]>=TOP_Q
    if state=="RANGE_ATR_COMPRESSION_BOTTOM20":
        return p["range_atr_rank"]<=BOTTOM_Q
    if state=="RANGE_ATR_EXPANSION_TOP20":
        return p["range_atr_rank"]>=TOP_Q
    if state=="COMPOSITE_COMPRESSION":
        return (p["rv10_rv60_rank"]<=BOTTOM_Q)&(p["range_atr_rank"]<=BOTTOM_Q)
    if state=="COMPOSITE_EXPANSION":
        return (p["rv10_rv60_rank"]>=TOP_Q)&(p["range_atr_rank"]>=TOP_Q)
...
def build_readiness(evidence:pd.DataFrame,years:pd.DataFrame,months:pd.DataFrame)->pd.DataFrame:
    ...
    # Composite states are allowed a smaller fixed minimum because intersection states are sparser.
    composite=out["state"].str.startswith("COMPOSITE_")
    ...
    out["development_ready_transition_state"]=out[gates].all(axis=1)
    return out.sort_values(
        ["development_ready_transition_state","mean_r_uplift_vs_complement","mean_r"],
        ascending=[False,False,False]
    ).reset_index(drop=True)

def run_lab(cfg:VolatilityTransitionConfig)->dict[str,Any]:
    root=Path(cfg.project_root).expanduser().resolve()
    ...
    evidence.to_csv(outdir/"volatility_transition_evidence.csv",index=False)
    years.to_csv(outdir/"volatility_transition_year_evidence.csv",index=False)
    months.to_csv(outdir/"volatility_transition_month_evidence.csv",index=False)
    ready.to_csv(outdir/"volatility_transition_readiness.csv",index=False)
    p[[
        "symbol","as_of","entry_date","rv_10","rv_20","rv_60","range_atr",
        "rv10_to_rv60","rv10_to_rv20","rv10_rv60_rank","rv10_rv20_rank",
        "range_atr_rank","r_multiple"
    ]].to_csv(outdir/"point_in_time_volatility_transition_panel.csv.gz",index=False,compression="gzip")

    best=ready[ready["development_ready_transition_state"]==True].head(1)
    report=[
        "# M77.32 Volatility Compression / Expansion Transition Edge Discovery","",
        "## Frozen design","",
        "- Development evidence only through 2017-12-31.",
        "- Frozen M77.26.1 NEXT_OPEN / 5ATR / 3ATR / 60-session executable outcome.",
        "- Frozen M77.21 point-in-time volatility features.",
        "- No PROBABILITY_UP threshold, Top-K, or management retuning.",
        "- All transition ranks are same-date cross-sectional transforms.","",
        "## Readiness","",_md(ready),"",
        "## Year evidence","",_md(years,80),""
    ]
    (outdir/"VOLATILITY_COMPRESSION_EXPANSION_TRANSITION_REPORT.md").write_text("\n".join(report))
    ...
    summary={
        ...
        "transition_states_tested":list(TRANSITION_STATES),
        "fixed_top_quantile":TOP_Q,"fixed_bottom_quantile":BOTTOM_Q,
        "point_in_time_cross_sectional_ranks":True,
        "development_ready_transition_states":int(ready["development_ready_transition_state"].sum()),
        ...
        "next_step":"REVIEW DEVELOPMENT-ONLY VOLATILITY-TRANSITION EVIDENCE; ANY SURVIVOR IS HYPOTHESIS-GENERATING AND REQUIRES SEPARATE PROSPECTIVE GOVERNANCE",
    }
    _atomic_json(outdir/"volatility_compression_expansion_transition_summary.json",summary)
    _atomic_json(outdir/"run_manifest.json",{
        "version":VERSION,
        "created_at":datetime.now(timezone.utc).isoformat(),
        "config":cfg.__dict__,
        "frozen_states":list(TRANSITION_STATES),
...
def build_parser()->argparse.ArgumentParser:
    p=argparse.ArgumentParser(description="M77.32 volatility compression/expansion transition edge discovery")
    p.add_argument("--project-root",required=True)
    return p

def main(argv:list[str]|None=None)->int:
    a=build_parser().parse_args(argv)
    print(json.dumps(run_lab(VolatilityTransitionConfig(project_root=a.project_root)),indent=2,sort_keys=True,default=_json_default))
    return 0
