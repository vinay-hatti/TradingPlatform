...
VERSION="M77.35.0-SERIAL-DEPENDENCE-PATH-SMOOTHNESS-EDGE-DISCOVERY-1.0"
...
STATES=(
    "POSITIVE_AUTOCORR_TOP20",
    "NEGATIVE_AUTOCORR_BOTTOM20",
    "UP_DAY_PERSISTENCE_TOP20",
    "DOWN_DAY_PERSISTENCE_TOP20",
    "SMOOTH_POSITIVE_PATH",
    "CHOPPY_POSITIVE_PATH",
    "MOMENTUM_ACCELERATION_TOP20",
    "MOMENTUM_DECELERATION_BOTTOM20",
)

class PathSmoothnessError(RuntimeError):
    pass

@dataclass(frozen=True)
class PathSmoothnessConfig:
    project_root:str
    ...
    output_dir:str="research_data/m77_35/serial_dependence_path_smoothness_edge_discovery"
...
def load_panel(cfg:PathSmoothnessConfig)->tuple[pd.DataFrame,dict[str,Any]]:
    ...
    if not ep.exists(): raise PathSmoothnessError(f"Executable Development panel missing: {ep}")
    if not ap.exists(): raise PathSmoothnessError(f"Frozen M77.21 feature authority missing: {ap}")
    ...
    if p.empty: raise PathSmoothnessError("Frozen executable cohort missing")
    if (p["as_of"]>DEVELOPMENT_END).any():
        raise PathSmoothnessError("M77.35 refuses post-2017 evidence")
    ...
    if missing:
        raise PathSmoothnessError(f"M77.21 feature authority missing required path features: {missing}")

    auth=auth[["symbol","as_of",*features]].copy()
    if auth.duplicated(["symbol","as_of"],keep=False).any():
        raise PathSmoothnessError("M77.21 feature authority is not unique on (symbol, as_of)")
    ...
    if (~complete).any():
        ...
        raise PathSmoothnessError(
            f"M77.35 point-in-time feature parity failed; missing_rows={int((~complete).sum())}, sample={detail}"
        )

    for c in (*features,"r_multiple"):
        p[c]=pd.to_numeric(p[c],errors="coerce").replace([np.inf,-np.inf],np.nan)

    # Same-date PIT cross-sectional ranks.
    p["autocorr_rank"]=p.groupby("as_of")["ret_autocorr_20"].rank(pct=True,method="average")
    p["upday_rank"]=p.groupby("as_of")["up_day_fraction_20"].rank(pct=True,method="average")
    p["downday_rank"]=p.groupby("as_of")["down_day_fraction_20"].rank(pct=True,method="average")
    p["momaccel_rank"]=p.groupby("as_of")["mom_accel_5_20"].rank(pct=True,method="average")

    # Path smoothness: positive intermediate return + persistent up-days + positive autocorrelation.
    p["smooth_positive_score"]=(p["autocorr_rank"]+p["upday_rank"])/2.0
    # Choppy positive path: positive 20d return but low/negative autocorrelation.
    p["positive_20d_rank"]=p.groupby("as_of")["px_ret_20"].rank(pct=True,method="average")
...
def _state_mask(p:pd.DataFrame,state:str)->pd.Series:
    if state=="POSITIVE_AUTOCORR_TOP20": return p["autocorr_rank"]>=TOP_Q
    if state=="NEGATIVE_AUTOCORR_BOTTOM20": return p["autocorr_rank"]<=BOTTOM_Q
    if state=="UP_DAY_PERSISTENCE_TOP20": return p["upday_rank"]>=TOP_Q
    if state=="DOWN_DAY_PERSISTENCE_TOP20": return p["downday_rank"]>=TOP_Q
    if state=="SMOOTH_POSITIVE_PATH":
        return (p["positive_20d_rank"]>=TOP_Q)&(p["smooth_positive_score"]>=TOP_Q)
    if state=="CHOPPY_POSITIVE_PATH":
        return (p["positive_20d_rank"]>=TOP_Q)&(p["autocorr_rank"]<=BOTTOM_Q)
    if state=="MOMENTUM_ACCELERATION_TOP20": return p["momaccel_rank"]>=TOP_Q
    if state=="MOMENTUM_DECELERATION_BOTTOM20": return p["momaccel_rank"]<=BOTTOM_Q
...
def build_readiness(evidence:pd.DataFrame,years:pd.DataFrame,months:pd.DataFrame)->pd.DataFrame:
    ...
    composite=out["state"].isin(["SMOOTH_POSITIVE_PATH","CHOPPY_POSITIVE_PATH"])
    ...
    out["development_ready_path_state"]=out[gates].all(axis=1)
    return out.sort_values(
        ["development_ready_path_state","mean_r_uplift_vs_complement","mean_r"],
        ascending=[False,False,False]
    ).reset_index(drop=True)

def run_lab(cfg:PathSmoothnessConfig)->dict[str,Any]:
    root=Path(cfg.project_root).expanduser().resolve()
    ...
    evidence.to_csv(outdir/"serial_dependence_path_smoothness_evidence.csv",index=False)
    years.to_csv(outdir/"serial_dependence_path_smoothness_year_evidence.csv",index=False)
    months.to_csv(outdir/"serial_dependence_path_smoothness_month_evidence.csv",index=False)
    ready.to_csv(outdir/"serial_dependence_path_smoothness_readiness.csv",index=False)
    p[[
        "symbol","as_of","entry_date","ret_autocorr_20","up_day_fraction_20",
        "down_day_fraction_20","mom_accel_5_20","mom_accel_20_60",
        "px_ret_5","px_ret_20","px_ret_60","autocorr_rank","upday_rank",
        "downday_rank","momaccel_rank","smooth_positive_score","positive_20d_rank",
        "r_multiple"
    ]].to_csv(outdir/"point_in_time_path_state_panel.csv.gz",index=False,compression="gzip")

    best=ready[ready["development_ready_path_state"]==True].head(1)
    report=[
        "# M77.35 Serial Dependence / Path Smoothness Edge Discovery","",
        "## Frozen design","",
        "- Development evidence only through 2017-12-31.",
        "- Frozen M77.26.1 NEXT_OPEN / 5ATR / 3ATR / 60-session executable outcome.",
        "- Frozen M77.21 point-in-time path-quality features.",
    ...
    (outdir/"SERIAL_DEPENDENCE_PATH_SMOOTHNESS_REPORT.md").write_text("\n".join(report))
    ...
    summary={
        ...
        "path_states_tested":list(STATES),
        "fixed_top_quantile":TOP_Q,"fixed_bottom_quantile":BOTTOM_Q,
        "point_in_time_cross_sectional_ranks":True,
        "development_ready_path_states":int(ready["development_ready_path_state"].sum()),
        ...
        "next_step":"REVIEW DEVELOPMENT-ONLY SERIAL-DEPENDENCE/PATH-SMOOTHNESS EVIDENCE; ANY SURVIVOR IS HYPOTHESIS-GENERATING AND REQUIRES SEPARATE PROSPECTIVE GOVERNANCE",
    }
    _atomic_json(outdir/"serial_dependence_path_smoothness_summary.json",summary)
...
def build_parser()->argparse.ArgumentParser:
    p=argparse.ArgumentParser(description="M77.35 serial dependence / path smoothness edge discovery")
    p.add_argument("--project-root",required=True)
    return p

def main(argv:list[str]|None=None)->int:
    a=build_parser().parse_args(argv)
    print(json.dumps(run_lab(PathSmoothnessConfig(project_root=a.project_root)),indent=2,sort_keys=True,default=_json_default))
    return 0
