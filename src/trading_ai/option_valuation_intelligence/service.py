    ...
    def build(
        ...
        *,
        ...
        with self.session_factory() as s:
            ...
            latest_spot = {}
            for sym in symbols:
                pr=s.execute(select(PriceHistory).where(PriceHistory.symbol==sym).order_by(desc(PriceHistory.date))).scalars().first()
                if pr and pr.close: latest_spot[sym]=float(pr.close)
            features=[]
            for cr in rows:
                oo=opportunity_by_id.get(cr.opportunity_id)
                if oo:
                    features.append(contract_features(cr,oo.symbol,sector_by_symbol.get(oo.symbol,'UNKNOWN'),latest_spot.get(oo.symbol,0.0)))
            ...
            for row in rows:
                opp = opportunity_by_id.get(row.opportunity_id) or s.get(InstitutionalOpportunityModel, row.opportunity_id)
                if not opp:
                    continue
                inf = None
                if inf_pub:
                    ir = s.execute(
                        select(InflectionSnapshotModel).where(
                            ...
                            InflectionSnapshotModel.symbol == opp.symbol,
                        ).order_by(desc(InflectionSnapshotModel.snapshot_timestamp))
                    ).scalars().first()
                    inf = dict(ir.payload_json or {}) if ir else None

                sibling_rows = s.execute(
                    select(ContractRecommendationModel).where(
                        ContractRecommendationModel.opportunity_id == row.opportunity_id,
                        ContractRecommendationModel.contract_recommendation_id != row.contract_recommendation_id,
                    )
                ).scalars().all()
                siblings = [dict(r.payload_json or {}) for r in sibling_rows]

                opportunity_payload = {'direction': opp.direction, **dict(opp.payload_json or {})}
                latest_price = s.execute(
                    select(PriceHistory).where(PriceHistory.symbol == opp.symbol).order_by(desc(PriceHistory.date))
                ).scalars().first()
                if latest_price and latest_price.close:
                    opportunity_payload['underlying_price'] = float(latest_price.close)
                    opportunity_payload['underlying_price_date'] = str(latest_price.date)

                dealer_snapshot = s.execute(
                    select(DealerPositionSnapshotModel).where(
                        DealerPositionSnapshotModel.symbol == opp.symbol
                    ).order_by(desc(DealerPositionSnapshotModel.as_of_date))
                ).scalars().first()
                if dealer_snapshot:
                    opportunity_payload['dealer_score'] = float(dealer_snapshot.institutional_positioning_score)
                    opportunity_payload['dealer_snapshot_date'] = str(dealer_snapshot.as_of_date)
                    opportunity_payload['gamma_regime'] = dealer_snapshot.gamma_regime
                    if not opportunity_payload.get('underlying_price') and dealer_snapshot.spot_price:
                        opportunity_payload['underlying_price'] = float(dealer_snapshot.spot_price)

                feature=feature_by_contract.get(row.contract_recommendation_id,{})
                rv_context=relative_context.get(row.contract_recommendation_id,{})
                if rv_context.get('available'):
                    opportunity_payload['peer_implied_volatility']=rv_context['peer_median_iv']
                    opportunity_payload['relative_value_z_score']=rv_context['z_score']
                    opportunity_payload['relative_value_regime']=rv_context['relationship_regime']
                    opportunity_payload['relative_value_peer_count']=rv_context['peer_count']
                    opportunity_payload['sector']=feature.get('sector','UNKNOWN')
                    opportunity_payload['sector_etf']=sector_etf_by_symbol.get(opp.symbol)
                iv_for_event=feature.get('iv') or 0.30
                ev_context=event_context(opp.symbol,opportunity_payload,events,iv_for_event,date.today())
                ...
                contract_payload = dict(row.payload_json or {})
                ...
                result['segmentation']={
                    'sector':feature.get('sector','UNKNOWN'),'strategy':feature.get('strategy','UNKNOWN'),
                    'dte_bucket':feature.get('dte_bucket','UNKNOWN'),'moneyness':feature.get('moneyness_bucket','UNKNOWN'),
                    'liquidity_bucket':'HIGH' if feature.get('liquidity',0)>=75 else ('MEDIUM' if feature.get('liquidity',0)>=50 else 'LOW'),
                    'right':feature.get('right','UNKNOWN'),
                }
                if rv_context.get('available'):
                    s.add(OptionRelativeValueSnapshotModel(
                        relative_value_id=f'M69-RV-{uuid4().hex.upper()}',valuation_run_id=valuation_run_id,
                        contract_recommendation_id=row.contract_recommendation_id,symbol=opp.symbol,
                        sector=feature.get('sector','UNKNOWN'),peer_group=rv_context['peer_group'],
                        symbol_iv=rv_context['symbol_iv'],peer_median_iv=rv_context['peer_median_iv'],
                        divergence_pct=rv_context['divergence_pct'],z_score=rv_context['z_score'],
                        relationship_regime=rv_context['relationship_regime'],snapshot_timestamp=now,payload_json=rv_context))

                existing = s.execute(
                    select(OptionValuationSnapshotModel).where(
                        OptionValuationSnapshotModel.contract_recommendation_id == row.contract_recommendation_id,
                        OptionValuationSnapshotModel.state_hash == result['state_hash'],
                    )
                ).scalars().first()
                if existing:
                ...
                dp = s.execute(
                    select(InstitutionalDecisionSnapshotModel).where(
                        InstitutionalDecisionSnapshotModel.opportunity_id == row.opportunity_id
                    )
                ).scalars().first()
                if dp:
                ...
                for name, meta in result['component_coverage'].items():
                    coverage_counts[f'{name}:available' if meta['available'] else f'{name}:fallback'] += 1
            ...
            summary = {
                ...
                'requested_recommendation_count': len(rows),
                'built': len(results),
            ...
            status = 'READY' if results and coverage >= 45 and independent == len(results) and spread >= 1.0 else (
