...
class BacktestReport:
        ...
        return [
            {
                ...
                "expectancy": self.money(r["expectancy"]),
            }
            for r in rows
        ]
    ...
    def streak_metrics(self, trades):
        return ReportingAnalytics().streaks(trades)
    ...
    def monthly_heatmap_rows(self, trades, initial_capital=100000):
        analytics = ReportingAnalytics()
        rows = analytics.monthly_heatmap(trades, initial_capital)
    ...
    def regime_rows(self, trades):
        rows = ReportingAnalytics().regime_performance(trades)
        return self.metric_rows(rows, "regime")
    ...
    def score_calibration_rows(self, trades):
        rows = ReportingAnalytics().score_calibration(trades)
        return self.metric_rows(rows, "score_bucket")
...
#        drawdown_rows = self.drawdown_rows(equity_curve)
        ...
        extended_risk = self.extended_risk_metrics(trades, equity_curve, metrics)
        streaks = self.streak_metrics(trades)

        monthly_heatmap_rows = self.monthly_heatmap_rows(
            trades,
        ...
        rolling_rows = self.rolling_rows(equity_curve)

        drawdown_curve = analytics.drawdown_curve(equity_curve)
        ...
        monthly_chart = charts.bar_chart(
            [
                {
                    "month": f"{r['year']}-{m}",
                    "return": float(str(r[m]).split(">")[1].split("<")[0].replace("%", "")) / 100.0
                    if "<span" in str(r[m])
                    else 0.0,
                }
...
<div class="card">
    ...
    <div class="metric"><strong>Sharpe</strong>{float(metrics.get("sharpe_ratio", 0.0)):.2f}</div>
    <div class="metric"><strong>Sortino</strong>{float(metrics.get("sortino_ratio", 0.0)):.2f}</div>
    <div class="metric"><strong>Calmar</strong>{float(metrics.get("calmar_ratio", 0.0)):.2f}</div>
    <div class="metric"><strong>Payoff Ratio</strong>{float(metrics.get("payoff_ratio", 0.0)):.2f}</div>
</div>
...
<div class="card">
    ...
    <div class="metric"><strong>Ulcer Index</strong>{extended_risk.get("ulcer_index", 0.0):.2f}</div>
    <div class="metric"><strong>Omega</strong>{extended_risk.get("omega_ratio", 0.0):.2f}</div>
    <div class="metric"><strong>Tail Ratio</strong>{extended_risk.get("tail_ratio", 0.0):.2f}</div>
    <div class="metric"><strong>Recovery Factor</strong>{extended_risk.get("recovery_factor", 0.0):.2f}</div>
