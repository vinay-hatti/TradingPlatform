...
def _sign(right:str, convention:str)->float:
    """Return the governed option-type exposure sign.

    Public open interest does not identify the actual dealer side.  The proxy
    therefore needs an explicit call/put convention; assigning one sign to
    every contract makes aggregate gamma incapable of crossing zero.

    ``customer_long_proxy`` follows the common public OI convention of calls
    contributing positive gamma and puts negative gamma.  ``street_proxy`` is
    the dealer-side inverse.  ``unsigned_market_exposure`` remains available
    for magnitude-only analytics and intentionally cannot imply a gamma flip.
    """
    normalized=str(right or '').upper()
    if normalized in {'C','CALLS'}: normalized='CALL'
    if normalized in {'P','PUTS'}: normalized='PUT'
    if convention=='unsigned_market_exposure': return 1.0
    if convention=='customer_long_proxy': return 1.0 if normalized=='CALL' else -1.0
    # Public street proxy: inverse of the customer-long OI proxy.
    return -1.0 if normalized=='CALL' else 1.0
...
class InstitutionalMarketStructureEngine:
    ...
    def analyze(self,symbol:str,as_of:date,spot:float,rows:Iterable[dict[str,Any]],realized_volatility:float|None=None,source_table:str='option_contract_history',previous_snapshot:InstitutionalMarketStructureSnapshot|None=None)->InstitutionalMarketStructureSnapshot:
        ...
        assumptions=(
            ...
            'Gamma flip is model-derived by repricing option-type-signed aggregate gamma over a spot shock grid.',
